...
class Grid():
    ...
    def _find_next_visible_seat_in_direcion(self, x, y, dx, dy, max_distance=np.inf):
        if dx==0 and dy==0:
            return None
        xx = x
        yy = y
    ...
    def _find_all_visible_seats(self, x, y, max_distance=np.inf):
        ...
        for (dx, dy) in itertools.product((-1,0,1), repeat=2):
            seat = g._find_next_visible_seat_in_direcion(x, y, dx, dy, max_distance=max_distance)
            if seat is not None:
                visible.append(seat)
        return visible
    ...
    def cache_visible_seats(self, max_distance = np.inf):
        ...
        for (x, y) in self.iterate_grid():
                visible_seats[(x, y)] = g._find_all_visible_seats(x,y, max_distance=max_distance)
...
def iterate_solve_grid(orig_grid, max_distance, occupancy_limit):
    ...
    for (n,m) in grid.iterate_grid():
        ...
        if orig_grid.get(n,m) == "#":
            if num_occupied >=occupancy_limit:
                grid.set(n,m, "L")
                changed = True
    print(".", end='')
    return changed, grid
